---
title: "Testing single-read version of RAW and CLEAN workflows"
author: "Simon Grimm"
date: 2024-10-23
format:
  html:
...
I'm adapting mgs-workflow to take in single-read sequencing data. Here I'm checking if i) the output of the single-read RAW and CLEAN workflows look as expected, and if ii) the output of the paired-end version is the same as the output of the test run on `dev`.

The single-end dataset simply consists of the forward reads of our usual test dataset. Additionally, I work with the paired-end test dataset, run on the `single-read-raw` pipeline with `read_type == paired_end`, and the same dataset run with the `dev` branch of `mgs-workflow`. I compared the `qc` output of the two paired-end runs, and they are identical, suggesting that the addition of single-read functionality doesn't impede mgs-workflow runs on paired-end data.
```{python}
#| label: load-packages
#| include: false
import os
...
Let's compare the basic stats of the single-read version to the paired-end version. Most notable is that paired-end data loses more bases and reads through FASTP cleaning. Accounting for paired-end reads containing twice as many base pairs, cleaned paired-end read files contain 16.4% fewer bases than single-end read files. This is largely due to FASTP dropping low-quality read pairs, with the number of cleaned paired-end reads 18.5% lower than the number of cleaned single-end reads. See @tbl-compare-base-pairs and @tbl-compare-read-pairs for sample level values.

My current hypothesis for this pattern is that the reverse reads are very low quality, as seen in @fig-quality-base-stats-comparison, leading FASTP to drop many of the read pairs with a low-quality reverse read. I won't investigate this further for now, but it suggests we should use a new test dataset that has high-quality forward and reverse reads.
...
As expected, the single-read run looks very similar to the forward read of the paired-end run (@fig-quality-base-stats-comparison). However, the Phred scores of the reverse reads are quite bad, with scores consistently under 30 after position 20.
...
Again, the paired-end output of `single-read-raw` is identical to the original paired-end output of `qc_quality_sequence_stats.tsv`.
...
Plotting the mean phred score (@fig-quality-sequence-stats-comparison), the average Phred scores of the single-read output looks similar to the forward reads of the paired-end run. Notable there are fewer sequences in the cleaned paired-end forward read. Again, this can be explained by FASTP dropping read pairs where the reverse read was low quality (@fig-quality-cleaned-single-vs-forward).
...
#| fig-cap: Average phred scores of the paired-end run forward reads,\ncompared to single-read phred scores
