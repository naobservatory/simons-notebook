...
Looking at Kingdom-level composition in both Kraken and Bracken results, the single-read output looks as expected, i.e., similar to the paired-end output. The paired-end run and the test have the same results in the plots shown below, suggesting that the addition of single-read functionality doesn't impede mgs-workflow runs on paired-end data.

# Introduction

The single-end dataset simply consists of the forward reads of our usual test dataset. Additionally, I work with the paired-end test dataset, run on the `single-read-raw` pipeline with `read_type == paired_end`, and the same dataset run with the `run` workflow of mgs-workflow.
...
se_taxonomy_dir = os.path.join(se_results_dir, "taxonomy")
pe_taxonomy_dir = os.path.join(pe_results_dir, "taxonomy")
test_taxonomy_dir = os.path.join(test_results_dir, "taxonomy")

se_bracken_path = os.path.join(se_taxonomy_dir, "bracken_reports_merged.tsv.gz")
pe_bracken_path = os.path.join(pe_taxonomy_dir, "bracken_reports_merged.tsv.gz")
test_bracken_path = os.path.join(test_taxonomy_dir, "bracken_reports_merged.tsv.gz")

se_kraken_path = os.path.join(se_taxonomy_dir, "kraken_reports_merged.tsv.gz")
pe_kraken_path = os.path.join(pe_taxonomy_dir, "kraken_reports_merged.tsv.gz")
test_kraken_path = os.path.join(test_taxonomy_dir, "kraken_reports_merged.tsv.gz")
```
